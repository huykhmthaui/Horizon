  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="focus:outline-none" asChild>
        <button
          className="dropdown-button w-full text-md font-semibold px-3 flex
        ...
        >
          <p className="bold text-xl text-primary">{server.name}</p>
          <ChevronDown
            className={cn("h-5 w-5 ml-auto mt-0.5", mobileMode && "ml-1")}
          />
        </button>
      </DropdownMenuTrigger>
      <DropdownMenuContent className="w-56 text-xs font-medium text-black dark:text-neutral-400 space-y-[2px]">
        ...
        {isModerator && (
          <DropdownMenuItem
            onClick={() => {
              setTimeout(() => onOpen("createChannel", { server }), 0);
            }}
            className="px-3 py-2 text-sm cursor-pointer"
          >
            Create Channel
        ...
        {!isAdmin && (
          <DropdownMenuItem
            onClick={() => {
              setTimeout(() => onOpen("leaveServer", { server }), 0);
            }}
            className="text-rose-500 px-3 py-2 text-sm cursor-pointer"
          >
            Leave Server
